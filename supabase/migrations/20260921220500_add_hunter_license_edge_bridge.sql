-- Service-only Hunter License bridge for the Edge Function.
-- The Edge Function verifies the caller's Supabase session first and passes
-- the verified user UUID here. Client roles cannot execute this function.

CREATE OR REPLACE FUNCTION public.project_prime_hunter_license_for(p_user uuid, p_display_name text DEFAULT 'Hunter'::text, p_favorite_hunter integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
    ...
    v_tier integer;
begin
    if v_user is null or v_user = '00000000-0000-0000-0000-000000000000'::uuid then
...
revoke all on function public.project_prime_hunter_license_for(uuid,text,integer)
  from public, anon, authenticated;
grant execute on function public.project_prime_hunter_license_for(uuid,text,integer)
  to service_role;
