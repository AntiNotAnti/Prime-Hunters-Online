-- Hunter License bridge for Project Prime.
-- Keeps the existing EF-managed prime schema as the career source of truth.
-- Supabase Auth owns client identity; the bridge creates the matching prime
-- player/license/profile row and returns only that user's accepted career data.
...
begin
    ...
    -- Only seed a new Supabase-backed license. A recovered account must keep
    -- the profile that already belongs to it instead of being overwritten by
    -- the new device's local launcher defaults.
