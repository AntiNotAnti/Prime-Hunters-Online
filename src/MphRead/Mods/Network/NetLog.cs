...
namespace MphRead.Mods.Network
{
    ...
    public static class NetLog
    {
        ...
        public static void Snapshot(double time, Scene? scene = null)
        {
            ...
            sb.Append($"main={(scene?.Players ?? PlayerEntity.LegacyRegistry).MainPlayerIndex} ");
            sb.Append($"mode={(scene?.GameState ?? GameState.Current).Mode} matchTime={(scene?.GameState ?? GameState.Current).MatchTime:0.0} ");
            ...
            // did not.
            sb.Append($"matchState={(scene?.GameState ?? GameState.Current).MatchState} goal={(scene?.GameState ?? GameState.Current).PointGoal} ");
            ...
            {
                PlayerEntity? p = (scene?.Players ?? PlayerEntity.LegacyRegistry).Items[slot];
                if (p == null)
                {
                ...
                line.Append($"name={(scene?.GameState ?? GameState.Current).Nicknames[slot],-10} ");
                ...
                line.Append($"score={(scene?.GameState ?? GameState.Current).Points[slot]}/{(scene?.GameState ?? GameState.Current).TeamPoints[slot]}p ");
                line.Append($"{(scene?.GameState ?? GameState.Current).Kills[slot]}k{(scene?.GameState ?? GameState.Current).Deaths[slot]}d ");
