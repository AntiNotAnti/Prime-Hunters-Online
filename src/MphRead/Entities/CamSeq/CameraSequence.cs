...
namespace MphRead.Formats
{
    ...
    public class CameraSequence
    {
        ...
        public static CameraSequence? Current { get => GameState.Current.CameraSequences.Current; set => GameState.Current.CameraSequences.Current = value; }
        public static CameraSequence? Intro { get => GameState.Current.CameraSequences.Intro; set => GameState.Current.CameraSequences.Intro = value; }
            ...
            CameraSequenceHeader header, IReadOnlyList<RawCameraSequenceKeyframe> keyframes)
        {
            ...
            _scene = scene;
            InitialCamInfo.Random = scene.Random;
        }
        ...
        public void SetUp(CameraInfo camInfo, ushort transitionTime)
        {
            ...
            if (SequenceId > 3)
            {
                _scene.Players.Main.CloseDialogs();
                _scene.Players.Main.ResetCombatVisor();
            }
            _scene.Players.Main.HudEndDisrupted();
        }
        ...
        private EntityBase? GetKeyframeRef(short type, short id)
        {
            if (type == (short)EntityType.Player)
            {
                ...
                if (id < _scene.Players.PlayerCount)
                {
                    return _scene.Players.Items[id];
                }
                return null;
            }
