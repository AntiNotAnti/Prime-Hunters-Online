...
| Play | `HubPlayView` asks only **Multiplayer / Offline / Adventure**. Multiplayer opens `HubMultiplayerView` for **Quick Play / Server Browser / Custom Match**. Adventure is now a hub-native save-slot surface backed by `AdventureSave` and `AdventureLaunch`; Offline remains the last transitional Play face and is opened single-purpose with its old tabs removed. |
