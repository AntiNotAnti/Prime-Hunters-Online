...
| Screen | What it does |
|---|---|
| Start | a responsive FPS-style hub over the shared launcher backdrop. Desktop uses a left deployment rail, central hunter/deployment card and operative/status panel; compact layouts replace the rail with a two-row action grid. **Play**, **Servers**, **Custom**, **Clips**, **Settings**, **Support** and **Quit** route into the existing screens, so the visual shell does not duplicate launch/network state. The build/update line remains independently actionable at the top-right and first-run game-file setup still replaces normal navigation until data is ready. `Mods/Launcher/Gui/HubHomeView.cs`, `HubNavButton.cs`, `HubTheme.cs` |
| Play | the home **PLAY** action opens `HubPlayView`: **Quick Play**, **Online**, **Custom Match**, **Offline**, **Adventure**. Quick Play asks the same portable `ServerBrowserService` used by the browser, selects the lowest-latency compatible server with an open slot, shows the selection and joins through the shared join path. Online opens `HubServerBrowserView` with direct address, live rows, selected-session detail, hunter/suit and Back/Create/Refresh/Join. Offline/Story/Replays still use transitional `PlayScreen` faces while those final presentations migrate. |
