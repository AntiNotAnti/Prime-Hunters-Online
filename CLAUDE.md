...
| Play | the home **PLAY** action opens `HubPlayView`, a deployment chooser with **Online**, **Custom Match**, **Offline** and **Adventure**. Online opens `HubServerBrowserView`: a dedicated FPS-style browser with player name/direct address across the top, live server rows, selected-session detail plus hunter/suit on the right, and Back/Create/Refresh/Join at the foot. Discovery/probing/endpoint parsing/joining live in renderer-neutral `ServerBrowserService`; the transitional `PlayScreen` online face shares the same join service rather than carrying a second connection implementation. Offline/Story/Replays still use `PlayScreen` while their hub-native surfaces are migrated. |
