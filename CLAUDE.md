...
| Play | `HubPlayView` asks only **Multiplayer / Offline / Adventure**. Multiplayer opens `HubMultiplayerView`, which contains **Quick Play / Server Browser / Custom Match**. Quick Play and Server Browser still share `ServerBrowserService`; Custom Match still enters the existing create/lobby flow. Offline and Adventure continue through the transitional `PlayScreen` faces while their final hub-native configuration surfaces are built. |
