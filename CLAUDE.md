...
| Screen | What it does |
|---|---|
| Start | a responsive FPS-style hub with **Play**, **Map Editor**, **Replay Studio**, **Settings** and **Quit**. Servers and Custom Match no longer duplicate Home actions; they live under Play → Multiplayer. Map Editor is a deliberate placeholder for the future visual workshop. The build/update line remains independently actionable and first-run game-file setup still replaces normal navigation until data is ready. |
| Replay Studio | `HubReplayStudioView` is a first-class Home destination rather than a Play tab. It lists full recordings and virtual clips, shows map/metadata detail, and exposes Watch, Import, Rename, Favorite, Check Integrity, Recover, Export, Delete and desktop Reveal Folder. Playback still enters the existing deterministic replay engine and in-match `ReplayControlsView`; the new screen is the library/workspace, not a second replay implementation. |
| Play | `HubPlayView` asks **Multiplayer / Offline / Adventure**. Multiplayer owns **Quick Play / Server Browser / Custom Match**. Offline is a hub-native map browser + local match setup backed by `OfflineLaunch`; Adventure is a hub-native save-slot/hunter flow backed by `AdventureSave` and `AdventureLaunch`. The old multipurpose Play screen remains only for legacy/replay/vote internals, not the primary Play information architecture. |
| Settings | the front hub opens `HubSettingsView`, a modern **Display / Audio / Controls / Replays / Profile / Credits** category surface. A category pushes the existing `SettingsView` already selected to that section, so staging, validation, Save/Cancel, gamepad capture and file/update behavior remain one implementation. In-match Settings still opens `SettingsView` directly to minimize steps while playing. |
| Custom Match | reached from **Play → Multiplayer → Custom Match**. `CreateServerScreen` keeps the existing hosted-vs-dedicated behavior, host discovery, dedicated-server install/start and map-rotation logic, but the visible flow is hub-native: **Lobby**, **Map Rotation**, **Hosting**, then Back / Install Server Files / Create Lobby. Map Rotation and Host selection use the same breadcrumb/action-bar shell rather than legacy standalone pages. |
| Lobby | the authoritative existing `LobbyScreen` is now laid out as an FPS lobby without changing its network state machine: **Roster** on the left, **Arena** and **Match Rules / owner actions** in the main area, full-width chat below, then Leave/Ready/Start. Narrow windows stack the regions in a scroller. Ready/start visibility and match edits are still read from `NetSession`/server session state, not copied into presentation state. |
...
| Pause | `PauseMenuView` uses hub-native actions over the existing scrim, so the real match stays visible. A live online session explicitly says **GAMEPLAY CONTINUES** rather than implying a network match stopped. Resume, vote, spectate/rejoin, fullscreen, recording, settings, leave and quit still raise the same host-owned events; the visual rewrite decides none of them itself. |
...
`-uishot DIR` photographs all of it with no display, including the FPS
`hub-home`, desktop/phone `deploy` layouts and populated `hub-servers`
layouts in addition to the transitional Play faces, create-server pages,
settings, setup, pause/results and the low-level server-row fixture. CI uploads
these as the `launcher-layouts` artifact. `-shellshot DIR` is the
