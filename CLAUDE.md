...
Replay ownership: Studio and killcams share `PassiveReplayPlayer` and instance-owned file/transport/decoder state. `Scene` owns player/match state, RNG, camera sequences, projectile pools, replication, mutable model/material/node/mesh state and portal/collision activation. Replica stepping never rebinds foreground facades, polls live input, opens sockets or resolves damage. Only the format diagnostic contains a legacy network-host adapter; production pose history and replay network smoothing are removed. See `.claude/multiplayer/NETWORK-DEMOS.md` and `docs/architecture/replay-map-upgrade-status.md` for contracts and acceptance evidence.

Map ownership: O(1) state-ID dirty tracking and bounded delta/coalesced history replace whole-map work for common edits. Fine invalidation retains per-object/imported/entity/selection/overlay caches. Stable CPU meshes feed the game renderer through `MapRenderFrame`; overlays composite above GPU geometry and only explicit captures read back. Logical/pixel camera math is shared by DPI, picking and captures. Details: `.claude/mapgen/MAP-STUDIO.md`.
