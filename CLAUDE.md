...
| Screen | What it does |
|---|---|
| Start | a responsive FPS-style hub with **Play**, **Map Editor**, **Replay Studio**, **Settings** and **Quit**. Map Editor opens the real Map Studio create/import/recovery workflow. Panels use restrained static gradients and contextual game imagery; page entry gets a short fade/lift unless **Reduce menu motion** is enabled. |
...
Replay ownership migration: `DemoPlayback` and `ReplayController` forward to an instance session/transport; only the theatre host bridges legacy globals. `Scene` owns `SceneGameState`, `ScenePlayerRegistry`, `MatchRandom`, camera sequences and beam pools. Entity simulation uses its owning scene; the static compatibility facades select the foreground scene only. `PassiveReplayScene` can load, step and render packet recordings through private replication histories, recorded health and silent audio without rebinding the foreground. Textures are GL allocated and scene owned; shared model lists have leases. `-replayreplicacheck FILE [-shots DIR]` checks two interleaved worlds, foreground sentinels and identical output after sibling disposal. Detached checkpoints now support the passive player; live capture and killcam/Studio attachment remain unfinished. See `.claude/multiplayer/NETWORK-DEMOS.md` and `docs/architecture/replay-map-upgrade-status.md` for remaining work.

Map build ownership: editor validation/navigation, builds, packaging, the package installer and synchronous client/server preparation share `MapBuildScheduler` (two workers, 32 jobs), a bounded private compiler cache and an integrity-checked runtime content cache. Cancelling a waiter leaves shared work running. Catalog locks must not be held while waiting for workers. `MapDependencyAnalyzer` supplies fingerprints and portable assets to packaging, package reference checks and Save As. Details: `.claude/mapgen/MAP-STUDIO.md`.

Map viewport: stable per-object CPU meshes feed the existing world shader/material/display-list renderer through `MapRenderFrame`. Avalonia draws entity/gizmo overlays and clears the viewport region in its shared bitmap; the GPU world composites beneath it. Only explicit preview capture reads GPU pixels. Common camera/layout math covers DPI, picking and captures; selection/camera/overlay changes do not upload geometry. `-mapviewportcheck DIR [-mapproject FILE]` checks window composition, pointer selection, mesh reuse and ownership on a real GL context, optionally profiling an imported map. Headless UI capture and renderer failures retain the CPU fallback.

Replay validation: gameplay hash schema 3 includes projectile/bomb/pickup state and gameplay RNG. Replica checks also compare a separate animation/trail/effect-particle projection per frame. Old hash schemas remain explicit compatibility boundaries without disabling packet playback. Timeline baselines retain match rules and held input fenced to occupant generation/life; submitted local input never establishes hit authority.

Detached replay checkpoints: `ReplayReplicaCheckpoint` preserves decoder/lifecycle/order/input-age state; `ReplayModelCheckpoint` uses asset identities and animation-group ordinals. `ReplayWorldCheckpoint` assembles the explicit multiplayer entity/effect/clock/link contract in a bounded value graph, fenced to room content and the construction baseline. It restores into an unpublished replica and rebinds that scene's native resources. `PassiveReplayPlayer` provides a bounded cache and at most 120 seek steps per update for files and frozen world clips. The replica check exercises immediate hashes/images, continued simulation, backward seeks and clips surviving timeline reset. Live checkpoint capture, killcam/Studio attachment and broader mode/runtime acceptance remain outstanding.

`-replayworldcheck FILE [-output DIR]` uses the source room/spawn for synthetic eight-actor, all-hunter fixtures across all 12 multiplayer modes. It runs the detached restore/continuation/seek checks through weapon, alt-form, affliction, death and respawn transitions. Effects are rebound by asset element ordinal, not a supposedly unique name. This is deterministic synthetic coverage, not live combat acceptance.

Live capture: `ReplayLiveWorld` consumes accepted recorder facts in its own continuously stepped replica and emits world checkpoints every 300 frames. Pending facts are bounded to 8,192/4 MiB; timeline retention is at least 45 seconds, grows for clip preferences, and remains capped at 64 MiB. Reset schedules resource disposal on the scene owner; failures invalidate capture until reset. `-replaylivecheck FILE` covers accepted facts → checkpoint → frozen clip → source reset → continuation/backward seek. Consumer migration remains in progress.

Replay killcams: `KillCam` is a boundary adapter for `KillcamController`; production defaults to the private replay scene. Live simulation continues. Personal clips fit the respawn window; final clips have explicit causal/timed eligibility. Input, private HUD font and versioned audio ownership are covered by `-replaykillcamcheck`. Legacy pose history remains only as a temporary developer fallback pending larger-match/Android acceptance.

Shared replay consumers: normal Studio now routes through `PassiveReplayPlayer`, without entering the live `NetSession` playback mode. Client/server recordings and instant clips share accepted timeline facts and write v4 exact initial worlds; v2/v3 remain readable. Nested and legacy clip ranges preserve hidden warmup. Clip disk serialization runs on a worker, with GL cleanup on the owner. Camera selection cannot change replica RNG. Replay Lab explicitly detaches to offline practice. Verification tools now compare the production private player's gameplay and presentation projections. See the migration status for remaining interpolation/export/platform acceptance.

Replay presentation/export: `ReplayPoseStream` owns a bounded accepted-snapshot lookahead, separate from simulation and network timing. Body and camera interpolation fence lifecycle, form and teleports. Camera tracks accept fractional frames. Export manifests v2 produce native-size world/HUD targets and genuine 120 FPS half-frame images from 60 Hz simulation; `-replayexportcheck` checks repeated image equality across cadence/seeks and verifies output dimensions. Keep export-status/transport overlays out of movies.
