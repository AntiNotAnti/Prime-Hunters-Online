...
Detached replay checkpoint components: `ReplayReplicaCheckpoint` preserves decoder/lifecycle/order/input-age state in a bounded versioned payload, with atomic validation before restore. `ReplayModelCheckpoint` uses asset names and animation-group ordinals. They contain no live scene/model/GPU references and are not full world restore points. Entity membership, links and effect/simulation assembly remain required before seek/killcam attachment.
