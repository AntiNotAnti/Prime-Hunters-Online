...
```bash
ProjectPrime -mapitems "ROOM"                       # from the recipe
ProjectPrime -mapitems level.pk3 -map NAME [-scale N]   # before there is one
```
...
```bash
ProjectPrime -q3convert path/to/level.pk3 -map LEVELNAME -name ROOM [-noclip] [-noitems]
```
...
ProjectPrime -mapcheck "DUST2"      # what it will be
ProjectPrime -mapgen "DUST2"        # in
```
...
ProjectPrime -mapitems "DUST2"             # what pickups the level already holds
ProjectPrime -mapcheck "DUST2"             # what its collision will be, before generating
...
- **the hash is the name.** Two people with a map called `de_dust2` do not
  ...
  cannot tell.
- **`51.161.113.128` is where they come from.** The directory already knows
