...
The library refreshes its own catalog and shows previews, metadata, source type, diagnostics and build status. Package and source precedence is identity-aware. Background jobs lock editing and publish UI results on the UI thread. Cancelling a caller stops its wait immediately; shared build work continues for other callers and the cache. Audit cancellation terminates only its child process.
