...
| `puppet smoothing: on, N.NN frames of buffer, jitter X ms` | the playout clock. The clean-line floor is 1.25 frames; differential packet-arrival jitter raises it continuously and starvation adds a temporary safety boost. `interpolated / held` is samples blended against samples that held a position |
...
| `pre-input puppet correction` / `presentation correction` | the first is the stale simulation displacement removed before local shooting input (at 60 Hz it should trend to zero); the second is the intentional high-refresh sub-frame move from current playout state to the exact world last shown |
